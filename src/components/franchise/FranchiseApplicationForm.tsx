  ...
  const onSubmit = async (data: FranchiseApplicationFormValues) => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(body?.error ?? 'Could not submit your application. Please try again.');
        return;
      }
      setSubmitted(true);
      reset();
    } catch {
      toast.error('Could not submit your application. Please check your connection and try again.');
    }
  };
